...
Artsdatabanken does have a webpage optimized for mobile phones, which also uses jQuery mobile. The customer wanted the application to have the same template as the webpage. To do that team used the CSS file from the webpage in application. In this way, the application is managed to look very similar to the mobile webpage.
\begin{figure}[h!]
 \begin{center}$
 \begin{array}{cc}
...
When the user makes a selection, the selected button becomes blue and the user becomes sure that he pressed the right button.
...
After selecting New Observation, user get into the window displayed in figure~\ref{fig:observation}. Here he selects the Species.  Their names are displayed, with pictures on the side. More species are listed; the user needs to slide his finger to see those.
...
After selecting the specie; bird, user gets a new window (figure~\ref{fig:bird}). This is the window where the information about the observation is added.
...
There are several buttons here for the user. If he wants to add more information about the specie, the “Add More Information” button is selected. This leads to a new window (shown in figure~\ref{fig:addmore}). To go back after entering new data, the user
